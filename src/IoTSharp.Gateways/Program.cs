...
{
    var result = await service.ExecuteReadAsync(
        operation.DriverCode,
        operation.ConnectionSettings,
        new DriverReadRequest(operation.Address, operation.DataType, operation.Length, operation.PointSettings),
        ct);
...
{
    var result = await service.ExecuteWriteAsync(
        operation.DriverCode,
        operation.ConnectionSettings,
        new DriverWriteRequest(operation.Address, operation.DataType, operation.Value, operation.Length, operation.PointSettings),
        ct);

    return Results.Ok(result);
});
app.MapPost("/api/runtime/poll/{taskId:guid}", async (Guid taskId, GatewayRuntimeService service, CancellationToken ct) =>
    Results.Ok(await service.ExecutePollingTaskAsync(taskId, ct)));
...
internal sealed record DriverReadOperation(
    string DriverCode,
    Dictionary<string, string?> ConnectionSettings,
    Dictionary<string, string?> PointSettings,
    string Address,
    GatewayDataType DataType,
    ushort Length = 1);

internal sealed record DriverWriteOperation(
    string DriverCode,
    Dictionary<string, string?> ConnectionSettings,
    Dictionary<string, string?> PointSettings,
    string Address,
